class Utilities{
    ...
    parse_useful_code(data, min_snippet_len=10){
      let code = new DOMParser().parseFromString(data, "text/html");        
      code = code.getElementsByTagName('code'); 

      if (code.length > 0){
          //code = code[0]      
          code = [...code].map(function (d) { return d.outerText;  });   
          code = code.filter(function (d) { return d.length > min_snippet_len;  });  
          return code                     
      }
